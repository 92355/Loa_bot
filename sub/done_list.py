...
import sqlite3
import asyncio
class done_list(commands.Cog):
    def __init__(self, app):
        self.app = app

    @commands.command(name = "완료4")
    async def complete_id(ctx, boss_num, msg):
        ...
        c.execute(f"UPDATE characters SET {boss_col} = ? WHERE character_name = ?", (1, msg))
        conn.commit()
        ...
        async for message in ctx.history(limit=10):
            if message.author == slef.bot.user and message.embeds:
                if len(characters) > 0:
...
async def setup(app):
    await app.add_cog(done_list(app))
